<template>
	<view class="all">
		...
		<view class="allbox">
			<view class="info">
				<view class="mainbox">
					<view class="txt1">{{$lang("您已赚取")}}</view>
					<view class="txt2">{{(infodata.money*rate).toFixed(2)}} USDT</view>
					<view class="txt3">{{$lang("邀请更多好友，赚取更多奖励")}}</view>
				</view>
			</view>
			<view class="history">
				<view class="txt1">{{$lang("邀请历史")}}</view>
				<view class="txt3">{{$lang("您已邀请过的好友")}}</view>
			</view>
			<view class="list">
				<view class="list-item" v-for="(i,index) in infodata.invite_list" :key="index">
					<view class="lpart">
						...
						<view class="nickname"> {{i.user?i.user.nickname:$lang("用户")}} </view>
					</view>
					<view class="rpart"> {{i.change_amount}}
						<image src="@/static/icons/gold.png" mode="aspectFill"></image> <u-icon name="arrow-right"
							color="#fbe3df" size="28"></u-icon>
					</view>
				</view>
			</view>
		</view>

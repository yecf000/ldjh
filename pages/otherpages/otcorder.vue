<template>
	<view>
		<navbar :title="$lang('订单')"></navbar>
		<view class="container">
			<!-- 流水列表 -->
			<view class="list-container">
				<view class="list-container-item" v-for="(i,index) in recordList" :key="index">
					<view class="item-line1">
						<view :style="{color:i.type==0?'#229516':'#c3000e'}" class="direction">
							{{i.type==0?$lang('买入'):$lang('卖出')}}</view>
						<!-- <view class="statu">{{i.status==1?$lang('已支付'):i.status==2?$lang('已完成'):i.status==3?$lang('取消'):i.status==4?$lang('申诉'):i.status==0?$lang('进行中'):$lang('异常')}}</view> -->
						<view class="statu">{{i.status_desc}}</view>
					</view>
					...
					<view class="item-line3">
						<view class="price">
							{{$lang('单价')}} {{i.entrust?i.entrust.price:'0.00'}}
						</view>
						<view class="total">
							<span
								style="font-weight: bold;margin-right: 8rpx;">{{i.entrust?(i.amount*i.entrust.price).toFixed(2):'0.00'}}</span>
							{{i.entrust?i.entrust.unit:'RUB'}}
						</view>
					</view>
					...
					<view class="item-line5">
						<view>{{i.store?i.store.title:$lang('商家')}}</view>
						<view class="message" @click="$u.route(`/pages/otherpages/impage?cid=${i.user.id}&ruid=${i.entrust?i.entrust.id:'0'}`)">
							<u-icon name="chat" color="#000" size="32"></u-icon>
						</view>
					</view>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			async getRecordList(isLoadMore = false) {
				...
				try {
					const res = await this.$reqGet("c2cUserLists", this.option)

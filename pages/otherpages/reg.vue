<template>
	<view class="all">
		<navbar title="注册" :backfuc='backfuc'></navbar>
		<view class="title">
			<view class="titletxt1">{{mode==1?'添加邮箱地址':'添加手机号'}}</view>
			<view class="titletxt2">此{{mode==1?'邮箱地址':'手机号'}}将成为您的登录账号，也是您访问我们提供的全部服务的关键。</view>
		</view>
		<view class="inputerbox">
			...
			<view class="inputitem">
				...
				<view class="input" :style="{border:input2border?'1px solid #fff':''}">
					<u-input type='password' placeholder='' :custom-style='inputstyle' v-model="password" @blur="input2border=false"
						@focus="input2border=true" />
				</view>
			</view>
			<view class="inputitem">
				<view class="title">邀请码(选填)</view>
				<view class="input" :style="{border:input3border?'1px solid #fff':''}">
					<u-input placeholder='' :custom-style='inputstyle' v-model="invite_code" @blur="input3border=false"
						@focus="input3border=true" />
				</view>
			</view>
			...
			<view class="tologin">
				已有账号？<span @click="$u.route('/pages/otherpages/login');">登录</span>
			</view>
		...
		methods: {
			...
			varify(){
				...
				if(!this.validatePassword(this.password)){
					this.$u.toast('密码必须包含字母和数字，并且大于6位')
					return false
				}	
				return true
			},

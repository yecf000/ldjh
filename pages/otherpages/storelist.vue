 <template>
 	<view class="">
 		<navbar :title="$lang('商户列表')"></navbar>
 		<view class="tabs">
 			<view @click="$u.route('/pages/otherpages/c2c')" class="tabitem">{{$lang('快捷交易')}}</view>
 			<view class="activetabitem">{{$lang('商户列表')}}</view>
 		</view>
 		...
 		<view class="listbox">
 			...
 			<view class="item" :style="{border:index+1==shops.length?'none':''}" v-for="(item,index) in shops"
 				:key="index">
 				<view class="itemtxt1">{{item.store?item.store.title:$lang('未命名商家')}}</view>
 				<view class="itemtxt2">{{item.store?item.store.order_count||0:0}} {{$lang('单')}}
 					{{item.store?item.store.success_rate||0:0}}% {{$lang('成交率')}}
 				</view>
 				<view class="itemtxt3">{{item.price}} {{item.unit.toUpperCase()}}</view>
 				<view class="itemtxt4">{{$lang('剩余数量')}} <span>{{item.left_amount}} {{$lang('金矿')}}</span></view>
 				<view class="itemtxt5">{{$lang('限额')}}
 				...
 				<view class="itemtxt6">
 					...
 					<view class="buybtn" @click="tradeready(item)">{{current==1?$lang('出售'):$lang('购买')}}</view>
 				</view>
 			</view>
 		</view>
 		...
 		<u-popup v-model="showbuyready" mode="bottom" border-radius="18">
 			<view class="popup">
 				<view class="popupnav">
 					<view>
 						<view class="title">{{$lang('购买金矿')}}</view>
 						<view class="uni">{{$lang('单价')}} ¥{{popinfo.price}}</view>
 					</view>
 				...
 				<view class="tradebox">
 					...
 					<view class="limit">
 						{{$lang('限额')}} {{$fuc.formatNumber(popinfo.min_amount)}} -
 						{{$fuc.formatNumber(popinfo.max_amount)}} {{popinfo.unit}}
 					</view>
 					<view class="couldget">
 						{{$lang('可得')}} <span>{{buytradenum?(buytradenum/popinfo.price).toFixed(2):'0.00'}}
 							{{$lang('金矿')}}</span>
 					</view>
 				</view>
 				<view class="tradebox">
 					<view class="paytitle">{{$lang('支付方式')}}</view>
 					<view class="payname">
 						<view>{{$lang('支付宝')}}</view>
 						<view class="type">{{$lang('即时付款')}}</view>
 					</view>
 				</view>
 				<view class="storeinfo">
 					...
 					<view class="infoitem">
 						<view class="itemlpart">{{$lang('商家昵称')}}</view>
 						<view class="itemrpart">{{popinfo.store?popinfo.store.title:$lang('未知商家')}}</view>
 					</view>
 					<view class="infoitem">
 						<view class="itemlpart">
 							<view>{{$lang('交易提醒')}}</view>
 							<view class="tips">{{$lang('交易提醒')}}</view>
 						</view>
 					</view>
 				</view>
 				<view class="buybtbox">
 		...
 		<u-popup v-model="showsellready" mode="bottom" border-radius="18">
 			<view class="popup">
 				<view class="popupnav">
 					<view>
 						<view class="title">{{$lang('出售金矿')}}</view>
 						<view class="uni">{{$lang('单价')}} ¥{{popinfo.price}}</view>
 					</view>
 				...
 				<view class="tradebox">
 					<view class="inputer">
 						<u-input :placeholder="'0.00 '+$lang('金矿')" :custom-style="{fontSize:'60rpx'}" type='number'
 					...
 					<view class="clouduse">
 						{{$lang('当前可售出金矿数量')}} {{userinfo.user_money}}
 					</view>
 					<view class="couldget">
 						{{$lang('可得')}} <span>{{selltradenum?(popinfo.price*selltradenum).toFixed(2):'0.00'}}
 							{{(popinfo.unit)}}</span>
 					</view>
 				</view>
 				<view class="tradebox">
